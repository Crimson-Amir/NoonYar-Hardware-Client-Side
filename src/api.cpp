...
const char *endpoint_address = "http://api.noonyar.ir:8000/hc";
...
int apiNewCustomer(const std::vector<int> &breads)
{
    ...
    JsonObject req = bodyDoc["bread_requirements"].to<JsonObject>();
...
NewBreadResponse apiNewBread()
{
    ...
    if (!doc["bread_index"].isNull())
    {
        r.bread_index = doc["bread_index"] | -1;
    }

    if (!doc["next_customer"].isNull())
    {
        r.next_customer = doc["next_customer"] | false;
    }

    if (!doc["customer_breads"].isNull())
    {
        JsonObject breads = doc["customer_breads"].as<JsonObject>();
...
bool apiInitCookDisplayFromServer()
{
    ...
    bool hasCustomerKey = !doc["has_customer"].isNull();
    bool hasCustomer = hasCustomerKey ? (bool)doc["has_customer"] : true;
    ...
    if (!doc["customer_breads"].isNull() && doc["customer_breads"].is<JsonObject>())
    {
        JsonObject breads = doc["customer_breads"].as<JsonObject>();
...
CurrentTicketResponse apiCurrentTicket()
{
    ...
    if (ready == true)
    {
        if (!doc["current_user_detail"].isNull() && doc["current_user_detail"].is<JsonObject>())
        {
